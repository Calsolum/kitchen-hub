...
- **Physical label printing** — a Bluetooth LE label printer (Niimbot B1-Pro) wired up as its own Flask service; print a Grocy-linked product label (name + soonest expiry, pulled live from stock) or a free-form text label, straight from the dashboard, with a live WYSIWYG preview letterboxed in a landscape frame, text that auto-sizes the title and subtitle independently so one long word never shrinks everything else, a live BLE connection indicator with a manual Connect button, a print queue with progress and cancellation, one-tap "purchased/opened/prepared today" date presets, and a built-in on-screen keyboard (the kiosk has no physical one)
...
- **A dropdown menu with a high `z-index` still rendering behind other content**: the "More" overflow menu lived inside the header bar, which (like the dashboard's cards) uses `backdrop-filter` for its glass effect — and `backdrop-filter` creates its own CSS stacking context regardless of `z-index`. The menu's `z-index:500` only ever competed against other elements *inside the header's own stacking context*; it never actually competed against the cards below, which each create their own separate stacking context. No `z-index` value fixes that from inside the wrong context. The real fix was moving the menu's markup out of the header entirely (to a sibling of the card grid, `position:fixed` instead of `absolute`) so its `z-index` finally compares against the right elements.
...
- **Auto-sized text that measured height but not width**: after making label text auto-scale to fill the sticker, a long single word (e.g. "Japanese") could still come back wider than the label at a large size and get clipped — the tier-picking logic checked that the wrapped block's total *height* fit, but never checked that each line's rendered *width* did too, since the word-wrapper can't split a word mid-way. Fixed by rejecting any font-size tier where any line's measured width exceeds the label, not just checking the height sum.
