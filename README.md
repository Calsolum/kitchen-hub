...
- **Physical label printing** — a Bluetooth LE label printer (Niimbot B1-Pro) wired up as its own Flask service; print a Grocy-linked product label (name + soonest expiry, pulled live from stock) or a free-form text label, straight from the dashboard, with a live WYSIWYG preview (rotated to match the label's actual landscape orientation), auto-sized text that fills the sticker instead of a fixed small font, a live BLE connection indicator, a print queue with progress and cancellation, one-tap "purchased/opened/prepared today" date presets, and a built-in on-screen keyboard (the kiosk has no physical one)
